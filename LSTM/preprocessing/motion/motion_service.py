...
from preprocessing.motion.motion_collection import MotionCollection
...
class MotionService(object):
    ...
    def load(motion_file, delimiter=','):
        motion_array = pd.read_csv(str(motion_file), delimiter=delimiter).dropna()
    ...
    def write(motion_collection):
        ...
        np.savetxt(motion_output_path, motion_collection.data, fmt='%f',delimiter=',')
    ...
    def get_raw_file_path(subject_id):
        return Constants.MOTION_FILE_PATH.joinpath(subject_id + '_motion.csv')